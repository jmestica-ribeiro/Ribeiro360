...
// ── Deduplicación por ciclo de guardado ────────────────────────────────────────
// Si el mismo usuario ya recibió un email para este hallazgo en los últimos 10s
// (ej. porque está asignado a dos roles), se omite el email duplicado pero
// la notificación interna (nc_notificaciones) se inserta igual.
const _emailedInCycle = new Map(); // hallazgoId → Set<userId>

function _markEmailed(hallazgoId, userIds) {
  if (!_emailedInCycle.has(hallazgoId)) {
    _emailedInCycle.set(hallazgoId, new Set());
    setTimeout(() => _emailedInCycle.delete(hallazgoId), 10_000);
  }
  const set = _emailedInCycle.get(hallazgoId);
  userIds.forEach(id => set.add(id));
}

function _filterNotYetEmailed(hallazgoId, userIds) {
  const set = _emailedInCycle.get(hallazgoId);
...
export async function notificarAsignacion({ hallazgoId, hallazgoNumero, tipo, newIds, prevIds = [] }) {
  ...
  // Dedup: solo enviar email a usuarios que no fueron ya notificados en este ciclo
  const toEmail = _filterNotYetEmailed(hallazgoId, added);
  if (toEmail.length === 0) return;
  _markEmailed(hallazgoId, toEmail);

  supabase.functions
    .invoke('send-nc-email', {
      body: { userIds: toEmail, hallazgoNumero, tipo, hallazgoId },
    })
    .then(({ error: fnErr }) => {
      if (fnErr) console.warn('[notif] email error:', fnErr.message);
    })
    .catch(err => console.warn('[notif] email invoke failed:', err?.message));
}
 ...
 * @param {'avance'|'cerrado'|'verif_aprobada'|'verif_rechazada'} evento
...
export function notificarCambioEstadoHallazgo({ hallazgoId, hallazgoNumero, evento, nuevoPaso, userIds }) {
  ...
  supabase.functions
    ...
    .then(({ error: fnErr }) => {
      if (fnErr) console.warn('[notif] estado email error:', fnErr.message);
    })
    .catch(err => console.warn('[notif] estado invoke failed:', err?.message));
}

// ── notificarDocumentoSGI ──────────────────────────────────────────────────────
...
export function notificarDocumentoSGI({ userId, documentoTitulo, versionNumero, evento, documentoId }) {
  ...
  supabase.functions
    ...
    .then(({ error: fnErr }) => {
      if (fnErr) console.warn('[notif] sgi email error:', fnErr.message);
    })
    .catch(err => console.warn('[notif] sgi invoke failed:', err?.message));
}
...
export function notificarAccionIncidente({ userId, incidenteNumero, accionDescripcion, incidenteId }) {
  ...
  supabase.functions
    ...
    .then(({ error: fnErr }) => {
      if (fnErr) console.warn('[notif] inc email error:', fnErr.message);
    })
    .catch(err => console.warn('[notif] inc invoke failed:', err?.message));
}
...
export function notificarNuevoCurso({ cursoTitulo, cursoId, visRules = [], destinatarios = [] }) {
  ...
  supabase.functions
    ...
    .then(({ error: fnErr }) => {
      if (fnErr) console.warn('[notif] curso email error:', fnErr.message);
    })
    .catch(err => console.warn('[notif] curso invoke failed:', err?.message));
}
...
export function notificarNuevoEvento({ eventoTitulo, eventoId, eventoFecha, visRules = [] }) {
  supabase.functions
    ...
    .then(({ error: fnErr }) => {
      if (fnErr) console.warn('[notif] evento email error:', fnErr.message);
    })
