    ...
    const fetchData = async () => {
      try {
        ...
        if (stepsData && stepsData.length > 0) {
          ...
          // Fetch associated blocks for all active steps
          ...
          if (blocksError) {
             console.error("Error al cargar los bloques:", blocksError);
          }

          // Merge steps and blocks
          const stepsWithBlocks = stepsData.map(step => ({
            ...step,
            blocks: (blocksData || []).filter(b => b.step_id === step.id)
          }));

          setSteps(stepsWithBlocks);
        }
      } catch (err) {
        console.error("Error al cargar el onboarding de Supabase:", err);
      } finally {
        setIsLoading(false);
      }
    };
