...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchEventos = async () => {
      ...
      const visMatchesEvento = (eventoId) => {
        const rules = (eventosVisRes.data || []).filter(r => r.evento_id === eventoId);
        if (rules.length === 0) return true;
        return rules.every(r => { const v = profile?.[r.campo]; return v && v.toLowerCase() === r.valor.toLowerCase(); });
      };
      const filteredEventos = (eventosRes.data || []).filter(e => visMatchesEvento(e.id)).slice(0, 3);
    ...
    const fetchCursos = async () => {
      ...
      const allCourses = (coursesRes.data || []).filter(c => visMatchesCurso(c.id));
