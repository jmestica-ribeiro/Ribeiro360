...
import { Search, Mail, MapPin, User, MessageCircle, Send, X, Filter, ChevronLeft, ChevronRight, Building2 } from 'lucide-react';
      ...
      <div className="directory-toolbar-advanced">
        <div className="search-row">
          ...
          {(activeDept !== 'Todos' || activeLocation !== 'Cualquiera' || searchTerm !== '') && (
            <button className="btn-clear-filters" onClick={() => { setActiveDept('Todos'); setActiveLocation('Cualquiera'); setSearchTerm(''); }}>
              Limpiar filtros
            </button>
          )}
        </div>
          ...
          <div className="filters-group-row">
              ...
              <div className="filter-section">
                ...
                <div className="dept-filters">
                  {departments.map(dept => (
                    <button key={dept} className={`filter-chip ${activeDept === dept ? 'active' : ''}`} onClick={() => setActiveDept(dept)}>
                      {dept}
                    </button>
                  ))}
                </div>
            ...
            {locations.length > 1 && (
              <div className="filter-section">
                ...
                <div className="dept-filters">
                  {locations.map(loc => (
                    <button key={loc} className={`filter-chip ${activeLocation === loc ? 'active' : ''}`} onClick={() => setActiveLocation(loc)}>
                      {loc}
                    </button>
                  ))}
                </div>
