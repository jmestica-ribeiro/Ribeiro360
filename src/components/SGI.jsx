...
const SGI = () => {
  ...
  const fetchCategories = async () => {
    setIsLoading(true);
    const { data } = await supabase.from('sgi_categorias').select('*').eq('activo', true).order('orden');
    if (data) {
      setCategories(data);
      
      const { data: allDocs } = await supabase.from('sgi_documentos')
        .select('id, categoria_id').eq('activo', true);
      ...
      const getRecursiveCount = (catId) => {
        ...
        subcategories.forEach(sub => {
          subDocs += getRecursiveCount(sub.id);
        });
        return directDocs + subDocs;
      };

      data.forEach(cat => {
        counts[cat.id] = getRecursiveCount(cat.id);
      });
  ...
  const fetchDocuments = async () => {
    ...
    const { data } = await supabase
      ...
      .eq('categoria_id', cat.id).eq('activo', true).order('codigo', { ascending: true });
    if (data) {
  ...
  const openEditDoc = async (doc) => {
    const { data: vigente } = await supabase.from('sgi_versiones').select('*')
      .eq('documento_id', doc.id).eq('vigente', true).maybeSingle();
    setEditingDoc(doc);
    setDocForm({
  ...
  const handleSaveDoc = async () => {
    ...
    if (docId) {
      ...
      const { data } = await supabase.from('sgi_documentos').insert({ ...docPayload, created_by: profile?.id }).select();
      docId = data?.[0]?.id;
    }

    if (docId && docForm.numero_version !== undefined) {
      // Subir archivo si se seleccionó uno
      let archivoPath = null;
      if (docForm._archivo) {
        const ext = docForm._archivo.name.split('.').pop();
        const fileName = `${docId}/rev${docForm.numero_version}_${Date.now()}.${ext}`;
        const { error: uploadError } = await supabase.storage
          .from('sgi-documentos')
          .upload(fileName, docForm._archivo, { upsert: true });
        if (!uploadError) archivoPath = fileName;
      }

      const verPayload = {
        documento_id: docId, numero_version: docForm.numero_version,
        fecha_emision: docForm.fecha_emision || null, notas_cambios: docForm.notas_cambios || null, vigente: true,
        ...(archivoPath && { archivo_url: archivoPath }),
      };
      if (docForm._ver_id) {
        await supabase.from('sgi_versiones').update(verPayload).eq('id', docForm._ver_id);
      } else {
  ...
  const handleDeleteDoc = async (docId) => {
    ...
    await supabase.from('sgi_documentos').update({ activo: false }).eq('id', docId);
                      ...
                      <>
                        ...
                        {doc.versionActual.archivo_url && (
                          <button
                            ...
                            onClick={async (e) => {
                              e.stopPropagation();
                              const { data } = await supabase.storage.from('sgi-documentos').createSignedUrl(doc.versionActual.archivo_url, 60);
